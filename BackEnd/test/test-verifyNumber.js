...
it("should return verificationId, message and 200 status code", async () => {
    let response = await request(fileServer).post(url).type("json").send({verificationId:verificationEntry.id, secreteNumber})
...
it("should return no verification entry and 401 status code", async () => {
        let response = await request(fileServer).post(url).type("json").send({verificationId:new ObjectId().toString(), secreteNumber})
...
it("should return message and 400 status code", async () => {
        let response = await request(fileServer).post(url).type("json").send({verificationId:verificationEntry._id, secreteNumber: "wrong number"})
        assert.equal(response.status, 401)
        assert.isString(response.body.message)
